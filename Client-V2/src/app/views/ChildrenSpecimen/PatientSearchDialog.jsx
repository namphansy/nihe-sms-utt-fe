...
import { searchByPage } from '../Patient/PatientServices';
...
class PatientSearchDialog extends React.Component {
  ...
  updatePageData = () => {
    ...
    searchByPage(searchObject).then(({ data }) => {
      this.setState({
        patientList: [...data.content], totalElements: data.totalElements
      }, () => {
      });
    }
    );
  ...
  render() {
    ...
    let columns = [
      ...
      { title: t("specimen.patientCode"), field: "patientCode", width: "150" },
      { title: t('specimen.patientName'), field: "fullName", align: "left", width: "150" },
      { title: t("specimen.patientPhone"), field: "phone", width: "150" },
      { title: t("specimen.patientAddress"), field: "address", width: "150" },
    ]

    return (
      <Dialog onClose={handleClose} open={open} PaperComponent={PaperComponent} maxWidth={'lg'} fullWidth={true}>
        <DialogTitle style={{ cursor: 'move' }} id="draggable-dialog-title">
          <span className="mb-20">{t("specimen.patientSelect")}</span>
        ...
        <DialogContent dividers>
          ...
          <Grid item xs={12}>
            <MaterialTable title='' data={this.state.patientList} columns={columns}
