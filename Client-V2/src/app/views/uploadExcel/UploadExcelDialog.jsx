...
import NotificationPopupImport from '../Component/NotificationPopup/NotificationPopupImport';
import EnterPasswordPopup from './EnterPasswordPopup';
import LoadingOverlay from "react-loading-overlay";
import "react-toastify/dist/ReactToastify.css";
import { saveListData } from "./UploadExcelService";
...
class UploadExcelDialog extends React.Component {
  state = {
    dragClass: "",
    files: [],
    statusList: [],
    ...
    shouldOpenEnterPasswordDialog: false,
    password: null,
    isLoading: false,
    dataUploadResponse: {},
    confirm: false,
  ...
  uploadSingleFile = index => {
    this.setState({ isLoading: true, shouldOpenEnterPasswordDialog: false })
    ...
    this.fileUpload(file.file).then(({ data }) => {
      if (data != null) {
        this.setState({
          ...
          confirm: true,
          numberDataSuccess: data.numberDataSuccess,
          dataUploadResponse: data,
          textPopup: <pre dangerouslySetInnerHTML={{ __html: data.text }} />
        });
      } else {
  ...
  openEnterPassword = () => {
    this.setState({ shouldOpenEnterPasswordDialog: true })
  }
          ...
          this.multipleFilesUpload(files)
            ...
            .catch((err) => {
              if (err.response?.data) {
                toast.error(t("general.upload_excel_error"));
  ...
  handleSaveListData = () => {
    this.setState({ isLoading: true, openPopupShowResult: false })
    saveListData(this.state.dataUploadResponse).then(({ data }) => {
      if (data != null) {
        this.setState({
          isLoading: false,
          confirm: false,
  ...
  render() {
    const { t, i18n, handleClose, handleSelect, selectedItem, open, confirmMessage } =
      this.props;
    let { dragClass, files, queProgress, shouldOpenShowStatusDialog, shouldOpenConfirmationDialog,
      showMessage, shouldOpenEnterPasswordDialog, isLoading } =
      this.state;
    ...
    return (
      ...
      >
        ...
        <DialogContent>
                      ...
                      <Grid item lg={4} md={4} sm={12} xs={12}>
                        <div className="flex">
                          <Button
                            ...
                            color="primary"
                            disabled={isLoading}
                            onClick={() => this.openEnterPassword()}
                          >
                            {t('general.upload')}
                          </Button>
                          <Button
          ...
          {shouldOpenEnterPasswordDialog && (
            <EnterPasswordPopup
              title={t("confirm")}
              open={shouldOpenEnterPasswordDialog}
              onConfirmDialogClose={this.handleDialogClose}
              onYesClick={() => this.uploadSingleFile(0)}
              text={confirmMessage}
              Yes={t("general.Yes")}
              No={t("general.No")}
        ...
        </DialogActions>
        {
          ...
          <NotificationPopupImport
            title={t('general.noti')}
            open={this.state.openPopupShowResult}
            onYesClick={this.handleSaveListData}
            onCancelClick={this.handleDialogClose}
            text={this.state.textPopup}
            size="lg"
            agree={t('general.agree')}
            confirm = {this.state.numberDataSuccess > 0 ? this.state.confirm : false}
          />
        }
      </Dialog>
    );
