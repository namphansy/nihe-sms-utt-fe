...
import { saveListDataResultClone } from './UploadExcelService';
...
class UploadEIDExcel extends React.Component {
  state = {
    dragClass: "",
    files: [],
    statusList: [],
  ...
  componentWillUnmount() {
    ...
    this.props.reloadReagentCode()
    this.props.reloadInternalTest();
  };

  handleSaveListData = () => {
    this.setState({ isLoading: true, openPopupShowResult: false })
    saveListDataResultClone(this.state.dataUploadResponse).then(({ data }) => {
      if (data != null) {
        this.setState({
          isLoading: false,
          confirm: false,
  ...
  render() {
    const { t, i18n, handleClose, handleSelect, selectedItem, open, confirmMessage } =
      this.props;
    let { dragClass, files, queProgress, shouldOpenShowStatusDialog,
      shouldOpenConfirmationDialog, showMessage, isLoading } = this.state;
      ...
      >
        ...
        </DialogActions>
        {
          ...
          <NotificationPopupImport
            ...
            onCancelClick={this.handleClosePopupShowResult}
